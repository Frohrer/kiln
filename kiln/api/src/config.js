...
const options = {
	...
	limit_overrides: {
		...
		validators: [(x) => !!x || `Failed to parse the overrides\n${x}`, validate_overrides],
	},
	proxy_domain: {
		desc: "Public domain URL for the proxy server",
		default: "http://localhost:2020",
		validators: [
			(x) => typeof x === "string" || `${x} is not a string`,
			(x) => {
				try {
					new URL(x);
					return true;
				} catch (e) {
					return `${x} is not a valid URL`;
				}
			},
		],
	},
...
function parse_overrides(overrides_string) {
	...
	for (const language in overrides) {
		...
		for (const key in overrides[language]) {
			if (!["max_process_count", "max_open_files", "max_file_size", "compile_memory_limit", "run_memory_limit", "compile_timeout", "run_timeout", "compile_cpu_time", "run_cpu_time", "output_max_size"].includes(key)) {
				return null;
			}
...
for (const option_name in options) {
	...
	const validator_parameters = env_val === undefined ? [value, value] : [parsed_val, env_val];
	const validation_response = apply_validators(option.validators, validator_parameters);
	if (validation_response !== true) {
		logger.error(`Config option ${option_name} failed validation:`, validation_response);
		process.exit(1);
	}
