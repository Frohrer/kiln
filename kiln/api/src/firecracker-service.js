...
class FirecrackerService {
    ...
    generateSetupScript(language, version) {
        ...
        const lines = [
            ...
            'mkdir -p /run',
            '',
            '# Add Debian backports for newer packages',
        ...
        switch(language) {
            case 'python':
                lines.push(
                    ...
                    `if ! command -v python${version.split('.').slice(0,2).join('.')} > /dev/null 2>&1; then`,
                    ...
                    `python${version.split('.').slice(0,2).join('.')} /tmp/get-pip.py`,
                    'rm /tmp/get-pip.py',
                    '',
                    '# Create symlinks',
                    `ln -sf /usr/local/bin/python${version.split('.').slice(0,2).join('.')} /usr/bin/python`,
